"""Main nutrition calculator — orchestrates OFF + AI fallback + cache."""
...
class NutritionCalculator:
    ...
    3. Fallback LLM si OFF ne retourne rien
    ...
    def calculate_recipe(
        ...
        logger.info("Ingredient texts: %s", ingredient_texts)
        ...
        for raw_text in ingredient_texts:
            if not raw_text or not raw_text.strip():
                logger.info("Skipping empty ingredient text: '%s'", raw_text)
                continue
    ...
    def _lookup_nutrition(self, food_name: str) -> tuple[Optional[NutritionFacts], Optional[float]]:
        """Cherche dans le cache, puis OFF, puis IA. Retourne (facts, typical_weight_g)."""
